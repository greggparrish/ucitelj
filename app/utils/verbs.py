...
SOFTENED_CONS = {'b': 'blj', 'c': 'č', 'd': 'đ', 'g': 'ž', 'h': 'š', 'k': 'č', 'l': 'lj', 'm':
                 'mlj', 'n': 'nj', 'p': 'plj', 's': 'š', 'sl': 'šlj', 'sn': 'šnj', 'st': 'šć',
                 't': 'ć', 'v': 'vlj', 'z': 'ž', 'zd': 'žđ', 'zn': 'žnj'}
...
class Conjugation:
    ...
    def present(self, third=False):
        ...
        self.p2 = '3P' if third is True else self.p2
        ...
        if self.p2 == '3S':
            ...
            c = f"{verb_root}{PRES_ENDINGS[self.p2]}"
        return c

    def past(self):
        ''' Past participle + conjugation of biti '''
        verb_root = self.form_root()
        return f"{verb_root}{PAST_PART[self.p3[1:]]} {BITI_FORMS[self.p2]}"

    def future(self):
        ''' Conjugation of htjeti + infinitive '''
        return f"{self.verb} {HTJETI_FORMS[self.p2]}"

    def conditional(self):
        ''' Past participle + conjugation of bih '''
        verb_root = self.form_root()
        return f"{verb_root}{PAST_PART[self.p3[1:]]} {BIH_FORMS[self.p2]}"
